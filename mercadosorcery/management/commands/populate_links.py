...
class Command(BaseCommand):
    help = 'Populates the database with card data from an external API'
    ...
    def handle(self, *args, **options):
        ...
        for card_data in all_card_data:
            ...
            # Evita duplicados verificando a combinação de nome e printing
            if not any(c.nome == card_name and c.printing == printing_value for c in cards_to_create):
                card = Carta(
                    nome=card_name,
        ...
        Carta.objects.bulk_create(cards_to_create)
        self.stdout.write(self.style.SUCCESS(f'Successfully populated the database with {len(cards_to_create)} cards.'))
